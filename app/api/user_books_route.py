...
def get_user_books():
    print('current_user', current_user)
    user_books_all = Book.query.filter(Book.user_id == current_user.id).all()
    user_books = user_books_all.order_by(desc(Book.id))
    user_books_dict = {user_book.id: user_book.to_dict()
...
def get_user_books():
    print('current_user', current_user)
    user_books = Book.query.filter(Book.user_id == current_user.id).order_by(desc(Book.created_at)).all()
    user_books_dict = {user_book.id: user_book.to_dict()
                       for user_book in user_books}
    return user_books_dict
