...
class CreateBook(FlaskForm):
    ...
    year = IntegerField('Year', validators=[DataRequired(), NumberRange(
        min=0, max=2022, message='Please provide a valid release year.')])
    ...
    description = StringField('Description', validators=[DataRequired(), Length(
        min=1,    max=5000, message='You have exceeded the maximum number of characters allowed.')])
    banned = StringField('Banned Reason', validators=[DataRequired(), Length(
        min=1, max=5000, message="You have exceeded the maximum number of characters allowed.")])
